...
public:
    ...
    OneEuroFilter(double freq=30, double mincutoff=1.0, double beta_=0.0, double dcutoff=1.0) {
        setFrequency(freq) ;
        setMinCutoff(mincutoff) ;
        setBeta(beta_) ;
        setDerivateCutoff(dcutoff) ;
        //mXFilt =  std::make_shared<LowPassFilter>(alpha(mincutoff)) ;
        //mDXFilt = std::make_shared<LowPassFilter>(alpha(dcutoff  )) ;
    }

    void initFilterParameter(double freq, double mincutoff=1.0, double beta_=0.0, double dcutoff=1.0) {
        setFrequency(freq) ;
        setMinCutoff(mincutoff) ;
        setBeta(beta_) ;
        setDerivateCutoff(dcutoff) ;
...
private:
    ...
    double mBeta ;
    double mDCutoff ;
    std::shared_ptr<LowPassFilter> mXFilt ;
    std::shared_ptr<LowPassFilter> mDXFilt ;
